...
class Migrations {
    ...
    class Migration6To7(val context: Context) : Migration(6, 7) {
        override fun migrate(database: SupportSQLiteDatabase) {
            ...
            if (appCur.moveToFirst()) {
                ...
                appMap.forEach {
                    database.execSQL("UPDATE `tbSequence` SET `_application_name`=? WHERE `_id`=?",
                    arrayOf<Any?>(it.value, it.key))
                }
            }
        }
    ...
    class Migration16To17 : Migration(16, 17) {
        override fun migrate(database: SupportSQLiteDatabase) {
            database.execSQL("ALTER TABLE `tbSequence` ADD COLUMN `_localPort` INTEGER")
        }
    }

    class Migration17To18 : Migration(17, 18) {
        override fun migrate(database: SupportSQLiteDatabase) {
            database.execSQL("CREATE TABLE `tbSequence_new` (`_id` INTEGER PRIMARY KEY AUTOINCREMENT, `_name` TEXT, " +
    ...
    class Migration18To19 : Migration(18, 19) {
        override fun migrate(database: SupportSQLiteDatabase) {
            database.execSQL("ALTER TABLE `tbSequence` ADD COLUMN `_ttl` INTEGER")
        }
    }
