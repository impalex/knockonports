...
private const val HTTP_TIMEOUT = 5000
...
fun getPublicIp(serviceUrl: String?, type: IpAddressType): String? {
    ...
    return try {
        val connection = URL(serviceUrl).openConnection().apply {
            connectTimeout = HTTP_TIMEOUT
            readTimeout = HTTP_TIMEOUT
        }
        try {
            val response = connection.getInputStream().bufferedReader().use { it.readText() }.trim()
